...
namespace MathSphere
{
    public partial class AdminSupport : System.Web.UI.Page
    {
        ...
        protected void rptArticles_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            var lit = (System.Web.UI.WebControls.Literal)e.Item.FindControl("litArticle");
            if (lit == null) return;
            ...
            string search = HttpUtility.HtmlAttributeEncode(title + " " + content);
...
<div data-art-card=""{HttpUtility.HtmlAttributeEncode(id)}""
     ...
     onclick=""asOpenModal('{titleJs}','{contentJs}','{dateJs}')"">
  <div class=""inline-block px-3 py-1 bg-primary/10 text-math-dark-blue rounded-full text-[10px] font-black uppercase tracking-widest mb-4"">Help Article</div>
  <h4 class=""text-base font-black text-math-dark-blue mb-2 leading-snug group-hover:text-math-blue transition-colors"">{HttpUtility.HtmlEncode(title)}</h4>
  <p class=""text-sm text-gray-500 font-medium leading-relaxed mb-5"">{HttpUtility.HtmlEncode(excerpt)}</p>
